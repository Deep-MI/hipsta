...
def createLabels(params):
    ...
    cmd = os.path.join(os.environ.get('FREESURFER_HOME'), "bin", "mri_binarize") + " " \
        + "--i " +  params.FILENAME + " " \
        + "--match " + " ".join([ str(x) for x in params.HSFLIST]) + " " \
        + "--o " + os.path.join(params.OUTDIR, "labels", params.HEMI + ".initial_labels.mgz")
    ...
    if params.internal.MERGE_MOLECULAR_LAYER is True:
        ...
        for i in range(np.shape(vxML)[0]):
            ...
            while np.shape(tmpML)[0]==0:
                ...
                if n>10 or (int((vxML[i,0]-n))<0) or (int((vxML[i,0]+n))>(vx.shape[0]-1)) or (int((vxML[i,1]-n))<0) or (int((vxML[i,1]+n))>(vx.shape[1]-1)) or (int((vxML[i,2]-n))<0) or (int((vxML[i,2]+n))>(vx.shape[2]-1)):
                    vxML[i,3]=0
                    break
...
def copy_labels_to_main(params):
    ...
    shutil.copyfile(
        params.FILENAME,
        os.path.join(params.OUTDIR, params.HEMI + ".labels.mgz"))
...
def mergeMolecularLayerRemove(params):
    """
    ...
    # check if ML is present in lookup table; if not, just copy
    # files
