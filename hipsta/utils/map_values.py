...
def _parse_arguments():
    ...
    required.add_argument('--label', dest='IN_LABEL', help=h_IN_LABEL, default=None, required=False, metavar='<file>')
    ...
    optional.add_argument('--integrate', dest='INTEGRATE', help=h_integrate, default='none', required=False, metavar= '<none|mode|median|mean|min|max>')
...
def mapValues(params, IN_VOL=None, IN_SURF=None, IN_LABEL=None, IN_INDICES=None, IN_SUFFIX='hsf', INTEGRATE='mode', SELECT=None, INTERP='nearest', writePSOL=False, writeMGH=False, writeANNOT=False):
    ...
    if subproc.returncode == 1:
        ...
        RESLICED_VOL = os.path.splitext(IN_VOL)[0] + "_reslicedToHighresHC" \
            + os.path.splitext(IN_VOL)[1]

        cmd = os.path.join(os.environ.get('FREESURFER_HOME'), "bin",
            "mri_convert") + " -rl " + IN_LABEL + " -rt " + INTERP + " " \
            + IN_VOL + " " + RESLICED_VOL
...
if __name__ == "__main__":
    ...
    mapValues(params=options.params, IN_VOL=options.IN_VOL, IN_SURF=options.IN_SURF, IN_LABEL=options.IN_LABEL, IN_INDICES=options.IN_INDICES, IN_SUFFIX=options.IN_SUFFIX, writePSOL=options.writePSOL, writeMGH=options.writeMGH, writeANNOT=options.writeANNOT, INTEGRATE=options.INTEGRATE, SELECT=options.SELECT, INTERP=options.INTERP)
