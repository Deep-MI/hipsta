...
def _tetra33(tmp1, tmp2, tmp12, ind1, ind2, ind3, v, A):
    ...
    a = (
        la.block_diag(np.ones((tmp1.shape[0], tmp1.shape[0])), np.ones((tmp1.shape[0], tmp1.shape[0])))
        + np.diag(np.ones(tmp1.shape[0]), k=3)
        + np.diag(np.ones(tmp1.shape[0]), k=-3)
    )
    a = a + np.diag(np.ones(a.shape[0]))
    ...
    if (ind1[0] == 0 and ind1[1] == 5 and ind2[0] == 2 and ind2[1] == 4 and ind3[0] == 1 and ind3[1] == 3) or (
        ind1[0] == 2 and ind1[1] == 3 and ind2[0] == 1 and ind2[1] == 5 and ind3[0] == 0 and ind3[1] == 4
    ):
        ...
        A = sp.coo_matrix(
            (
                np.ones(np.shape(A.nonzero())[1] + 1),
                (np.hstack((A.nonzero()[0], np.shape(v)[0] - 1)), np.hstack((A.nonzero()[1], np.shape(v)[0] - 1))),
            ),
...
def cutTetra(params):
    ...
    idx = np.concatenate(
        (t4c[:, [0, 1]], t4c[:, [0, 2]], t4c[:, [0, 3]], t4c[:, [1, 2]], t4c[:, [1, 3]], t4c[:, [2, 3]]), axis=0
    )
    ...
    for i in range(len(ilHead[0])):
        ...
        # therefore we add '-1' at the end.
        tmp2 = (
            llHead[0][np.where(jlHead[0] == ilHead[0][i])[0][:], :] + np.shape(v4)[0] + np.shape(vlTail[0])[0] - 1
        ).squeeze()
