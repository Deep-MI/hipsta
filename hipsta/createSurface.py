...
def extractSurface(params):
    ...
    if params.internal.MCA == "mri_mc":
        cmd = (
            os.path.join(os.environ.get("FREESURFER_HOME"), "bin", "mri_mc")
            + " "
        ...
        cmd = (
            os.path.join(os.environ.get("FREESURFER_HOME"), "bin", "mri_pretess")
            + " "
            + os.path.join(params.OUTDIR, params.HEMI + ".mask.mgz")
            + " xyz "
            + os.path.join(params.OUTDIR, params.HEMI + ".mask.mgz")
        ...
        cmd = (
            os.path.join(os.environ.get("FREESURFER_HOME"), "bin", "mri_tessellate")
            + " "
            + os.path.join(params.OUTDIR, "surface", params.HEMI + ".pretess.mgz")
            + " 1 "
        ...
        cmd = (
            os.path.join(os.environ.get("FREESURFER_HOME"), "bin", "mris_convert")
            + " "
            + os.path.join(params.OUTDIR, "surface", params.HEMI + ".surf.vtk")
            + " "
            + os.path.join(params.OUTDIR, "surface", params.HEMI + ".initial_surf.vtk")
    ...
    if params.internal.REMESH is True:
        ...
        if shutil.which("mris_remesh") is None:
            ...
            if len(removeEdges) > 0:
                removeTrias = np.unique(
                    [
                        np.where(np.sum(np.logical_or(trSort == i[0], trSort == i[1]), axis=1) == 2)[0]
            ...
            trSortRmBndEdges = np.concatenate(
                (trSortRmBnd[:, [0, 1]], trSortRmBnd[:, [1, 2]], trSortRmBnd[:, [0, 2]]), axis=0
            )
            ...
            countEdgesInTrias = np.array(
                [
                    np.sum(
                        np.sum(
                            np.logical_or(trSortRmBnd == trSortRmBndEdges[i, 0], trSortRmBnd == trSortRmBndEdges[i, 1]),
                            axis=1,
                        )
                        == 2
                    )
                    for i in range(0, len(trSortRmBndEdges))
                ]
            )
            ...
            if params.internal.REMESH_SIZE == 0:
                cmd = (
                    os.path.join(os.environ.get("FREESURFER_HOME"), "bin", "mris_remesh")
                    + " "
                    + "--remesh  -i "
                    + params.SURFNAME
                    + " "
                    + "-o "
                    + os.path.join(params.OUTDIR, "surface", params.HEMI + ".remeshed_surf.vtk")
                )
                ...
                cmd = (
                    ...
                    + " "
                    + "--nvert "
                    + str(params.internal.REMESH_SIZE)
                    + "  -i "
                    + params.SURFNAME
                    + " "
                    + "-o "
